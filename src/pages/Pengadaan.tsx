...
import { openPrintWindow, printIntoWindow, printHtml } from '@/lib/print';
...
async function printPO(poId: string) {
  // Open the window synchronously from the click handler so Chrome/Safari do not block it after async Supabase requests.
  const printWindow = openPrintWindow();
  if (!printWindow) return;
  try {
    const { data: po, error: poError } = await tenantFrom('purchase_orders').select('*, suppliers(*), purchase_order_items(*, medicines(*))').eq('id', poId).single() as any;
    if (poError || !po) throw new Error(poError?.message || 'Surat Pesanan tidak ditemukan.');
    const settings = await fetchSettings();
    printSuratPesanan(po, settings, printWindow);
  } catch (error) {
    try { printWindow.close(); } catch { /* ignore */ }
    alert(`Gagal menyiapkan Surat Pesanan: ${error instanceof Error ? error.message : 'Terjadi kesalahan.'}`);
  }
}

function printSuratPesanan(po: any, settings: Settings, printWindow?: Window) {
  ...
  const itemRows = po.purchase_order_items.map((item: any, idx: number) => {
    ...
    return `
      <tr>
        ...
        ${(po.sp_type === 'prekursor' || po.sp_type === 'oot') ? `<td style="padding:6px 8px;border:1px solid #ccc;">${po.sp_type === 'prekursor' ? (item.precursor_active_ingredient ?? med?.generic_name ?? med?.description ?? '-') : (item.active_ingredient ?? med?.generic_name ?? med?.description ?? '-')}</td>` : ''}
  ...
  const activeIngredientHeader = (po.sp_type === 'prekursor' || po.sp_type === 'oot')
    ? `<th style="padding:6px 8px;border:1px solid #ccc;text-align:left;">${po.sp_type === 'prekursor' ? 'Zat Aktif Prekursor Farmasi' : 'Zat Aktif'}</th>` : '';
  ...
  if (printWindow) printIntoWindow(printWindow, html);
  else printHtml(html);
}
...
function POForm({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const [items, setItems] = useState<{ medicine: Medicine; quantity: number; unit: string; unitPrice: number; precursorActiveIngredient: string; activeIngredient: string }[]>([]);
  ...
  function addMed(med: Medicine) {
    ...
    setItems(prev => [...prev, { medicine: med, quantity: 1, unit: med.unit || 'pcs', unitPrice: med.buy_price, precursorActiveIngredient: med.generic_name ?? '', activeIngredient: med.generic_name ?? med.description ?? '' }]);
    setMedSearch('');
  }
  ...
  async function createNewMedicine() {
    ...
    if (spType === 'narkotika' && items.length >= 1) {
      ...
      setItems([{ medicine: med, quantity: 1, unit: med.unit || 'pcs', unitPrice: med.buy_price, precursorActiveIngredient: med.generic_name ?? '', activeIngredient: med.generic_name ?? med.description ?? '' }]);
    } else {
      setItems(prev => [...prev, { medicine: med, quantity: 1, unit: med.unit || 'pcs', unitPrice: med.buy_price, precursorActiveIngredient: med.generic_name ?? '', activeIngredient: med.generic_name ?? med.description ?? '' }]);
    }
    setShowNewMed(false);
    ...
    try {
      ...
      await tenantFrom('purchase_order_items').insert(
        items.map(i => ({
          ...
          ...(spType === 'prekursor' ? { precursor_active_ingredient: i.precursorActiveIngredient.trim() || null } : {}),
          ...(spType === 'oot' ? { active_ingredient: i.activeIngredient.trim() || null } : {}),
        }))
      );
      onSaved();
    } catch (err) {
                ...
                <tr key={item.medicine.id}>
                  <td className="px-3 py-2 font-medium text-gray-700">
                    ...
                    {(spType === 'prekursor' || spType === 'narkotika' || spType === 'oot') && (
                      spType === 'prekursor' ? (
                        <div className="mt-2">
                          <label className="block text-[11px] font-semibold text-amber-700 mb-1">Zat Aktif Prekursor Farmasi</label>
                          <input
                            type="text"
                            value={item.precursorActiveIngredient}
                            onChange={e => setItems(prev => prev.map((it, i) => i === idx ? { ...it, precursorActiveIngredient: e.target.value } : it))}
                            placeholder={item.medicine.generic_name ?? 'Masukkan zat aktif prekursor farmasi'}
                            className="w-full px-2 py-1.5 border border-amber-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-amber-300"
                          />
                          <p className="text-[10px] text-gray-400 mt-1">Nilai ini dapat dikoreksi tanpa mengubah data master obat.</p>
                        </div>
                      ) : (
                        <div className="mt-2">
                          <label className="block text-[11px] font-semibold text-blue-700 mb-1">Zat Aktif</label>
                          <input
                            type="text"
                            value={item.activeIngredient}
                            onChange={e => setItems(prev => prev.map((it, i) => i === idx ? { ...it, activeIngredient: e.target.value } : it))}
                            placeholder={item.medicine.generic_name ?? 'Masukkan zat aktif'}
                            className="w-full px-2 py-1.5 border border-blue-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-blue-300"
                          />
                          <p className="text-[10px] text-gray-400 mt-1">Nilai ini dapat dikoreksi khusus untuk SP OOT.</p>
                        </div>
                      )
                    )}
                  </td>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
              <tr>
                ...
                {(po.sp_type === 'prekursor' || po.sp_type === 'oot') && <th className="text-left px-3 py-2">{po.sp_type === 'prekursor' ? 'Zat Aktif Prekursor Farmasi' : 'Zat Aktif'}</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {po.purchase_order_items.map(item => (
                <tr key={item.id}>
                  ...
                  {(po.sp_type === 'prekursor' || po.sp_type === 'oot') && <td className="px-3 py-2 text-gray-600">{po.sp_type === 'prekursor' ? (item.precursor_active_ingredient ?? item.medicines?.generic_name ?? item.medicines?.description ?? '-') : ((item as any).active_ingredient ?? item.medicines?.generic_name ?? item.medicines?.description ?? '-')}</td>}
                </tr>
              ))}
            </tbody>
          </table>
