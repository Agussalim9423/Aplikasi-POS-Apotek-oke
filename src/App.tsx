...
> {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo) {
    console.error(`Error pada halaman ${this.props.pageName}:`, error, info);
  }
      ...
      return (
        <div className="p-6">
          <div className="max-w-2xl mx-auto bg-white border border-red-200 rounded-2xl shadow-sm p-6">
            <h2 className="text-lg font-bold text-red-600">Halaman {this.props.pageName} mengalami error</h2>
            <p className="mt-2 text-sm text-gray-600">
              Aplikasi tidak berhenti total. Detail error ditampilkan agar perbaikannya bisa dilakukan dengan tepat.
            </p>
            <pre className="mt-4 p-4 bg-gray-50 rounded-xl text-xs text-red-700 whitespace-pre-wrap overflow-auto">
              {this.state.error.message || String(this.state.error)}
            </pre>
            <button
              onClick={() => this.setState({ error: null })}
              className="mt-4 bg-teal-500 hover:bg-teal-600 text-white px-4 py-2 rounded-xl text-sm font-semibold"
            >
              Coba Muat Ulang Laporan
            </button>
          </div>
        </div>
      );
...
function AppContent() {
  ...
  return (
    ...
    >
      ...
      {effectivePage ===
        'laporan' && (
        <PageErrorBoundary pageName="Laporan">
          <Laporan />
        </PageErrorBoundary>
      )}
